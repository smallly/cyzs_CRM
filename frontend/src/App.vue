          ...
          <template v-if="activeMenu === 'contacts'">
            <div class="card">
              ...
              <div class="form-grid cols-3">
                ...
                <label class="field">
                  <span class="field-label">手机号（必填）</span>
                  <input v-model="contactForm.phone1" placeholder="请输入手机号1" />
                </label>
                <label class="field">
                  <span class="field-label">手机号（选填）</span>
                  <input v-model="contactForm.phone2" placeholder="请输入手机号2" />
                </label>
              </div>
              <div class="row">
                <button @click="saveContact">{{ contactEditingId ? "保存编辑" : "新增联系人" }}</button>
                <button v-if="contactEditingId" class="secondary" @click="cancelContactEdit">取消编辑</button>
              ...
              <table>
                ...
                <tbody>
                  <tr v-for="c in contacts" :key="c.id">
                    ...
                    <td>{{ c.ownerId || "-" }}</td>
                    <td>{{ c.creatorId || "-" }}</td>
                    <td>{{ c.deleted ? "true" : "false" }}</td>
                    <td>{{ formatDateTime(c.createdAt) }}</td>
                    <td>{{ formatDateTime(c.deletedAt) }}</td>
                    <td>
                      <div class="row">
                        <button class="secondary" @click="startContactEdit(c)">编辑</button>
                        <button class="secondary" @click="deleteContact(c.id)">删除</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
              ...
              <table>
                ...
                <tbody>
                  <tr v-for="p in projects" :key="p.id">
                    ...
                    <td>
                      <div class="row">
                        <button @click="changeProjectStage(p)">改阶段</button>
                        <button class="secondary" @click="transferProjectOwner(p)">转负责人</button>
                        <button class="secondary" @click="deleteProject(p.id)">删除</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
          ...
          <template v-if="activeMenu === 'project-create'">
            <div class="card">
              ...
              <div class="form-grid cols-4">
                ...
                <label class="field">
                  <span class="field-label">项目负责人（必填）</span>
                ...
                <label class="field">
                  <span class="field-label">首次建联时间</span>
                  <input v-model="projectForm.firstContactAt" type="datetime-local" />
                </label>
                <label class="field">
                ...
                <label class="field">
                  <span class="field-label">首次带看日期</span>
                  <input v-model="projectForm.firstVisitDate" type="date" />
                </label>
                <label class="field">
                  <span class="field-label">首次谈判日期</span>
                  <input v-model="projectForm.firstNegotiationDate" type="date" />
                </label>
                <label class="field">
                  <span class="field-label">入驻日期</span>
                  <input v-model="projectForm.movedInDate" type="date" />
                </label>
                <label class="field">
          ...
          <template v-if="activeMenu === 'project-detail'">
            <div v-if="selectedProject" class="project-detail-page">
              <div class="card project-hero-card">
                <div class="project-hero-top">
                  ...
                  <div class="project-hero-actions">
                    <button class="secondary project-stage-btn" @click="updateSelectedProjectStage">更新阶段</button>
                    <button class="secondary" @click="openProjectEdit">编辑</button>
                    <button class="secondary" @click="transferSelectedProjectOwner">更换负责人</button>
                  </div>
                </div>
              ...
              <div class="card project-base-card">
                ...
                <div v-if="projectEditMode" class="project-edit-wrap">
                  <div class="form-grid cols-4">
                    <label class="field">
                      <span class="field-label">项目名称（必填）</span>
                ...
                <div v-else class="project-base-grid">
              ...
              <div class="detail-panel" v-if="projectDetailTab === 'followups'">
                <div class="detail-list-toolbar">
                  ...
                  <div class="detail-list-actions-inline">
                    <button class="secondary" @click="jumpToFollowupFromDetail">新增跟进</button>
                  </div>
                </div>
                    ...
                    <div class="followup-card-head">
                      ...
                      <div class="followup-head-main">
                        <div class="followup-head-title">
                          ...
                          <span class="followup-time">创建人：{{ formatDateTime(f.createdAt || f.followupAt) }}</span>
                        </div>
                      </div>
                    </div>
                ...
                <table class="detail-list-table">
                  ...
                  <tbody v-else>
                    <tr>
                      <td class="empty-row" colspan="6">没有数据</td>
                    </tr>
                  </tbody>
                </table>
                </div>
          ...
          <template v-if="activeMenu === 'followups'">
            <div class="card">
              ...
              <div class="form-grid cols-3">
                ...
                <label class="field">
                  <span class="field-label">跟进内容</span>
                  <input v-model="followupForm.content" placeholder="请输入跟进内容" />
                </label>
                <label class="field">
                  <span class="field-label">跟进时间</span>
                  <input v-model="followupForm.followupAt" type="datetime-local" />
                </label>
              </div>
              <div class="row">
                <button @click="createFollowup">新增跟进</button>
              ...
              <table>
                ...
                <tbody>
                  <tr v-for="u in users" :key="u.id">
                    ...
                    <td>
                      <div class="row">
                        <button @click="updateRole(u)">保存角色</button>
                        <button class="secondary" @click="updateStatus(u)">保存状态</button>
                      </div>
                    </td>
                  </tr>
                </tbody>
...
function formatAreaRange(project?: ProjectRow | null): string {
  ...
  if (min !== null && max !== null) {
    if (min === max) return formatAreaNum(min);
...
function parseAreaInput(raw: string, label: string): number | null {
...
async function loadContacts() {
  try {
    contacts.value = await api<ContactRow[]>("/api/contacts");
  } catch (e) {
    setError(e);
  }
}
...
async function saveContact() {
  try {
    if (contactEditingId.value) {
      ...
      await api<ContactRow>("/api/contacts", { method: "POST", body: JSON.stringify(contactForm) });
      setOk("联系人已创建");
    }
    resetContactForm();
    await loadContacts();
...
async function loadProjects() {
  try {
    projects.value = await api<ProjectRow[]>("/api/projects");
  } catch (e) {
    setError(e);
  }
}
async function createProject() {
  try {
    ...
    if (projectForm.firstContactAt) payload.firstContactAt = new Date(projectForm.firstContactAt).toISOString().slice(0, 19);
    if (projectForm.firstVisitDate) payload.firstVisitDate = projectForm.firstVisitDate;
    if (projectForm.firstNegotiationDate) payload.firstNegotiationDate = projectForm.firstNegotiationDate;
    if (projectForm.movedInDate) payload.movedInDate = projectForm.movedInDate;
...
async function updateSelectedProjectStage() {
  ...
  const currentIndex = Math.max(0, stageOptions.indexOf(selectedProject.value.stage as (typeof stageOptions)[number]));
...
async function transferProjectOwner(p: ProjectRow) {
...
async function loadFollowupsByProject() {
  ...
  try {
    followups.value = await api<FollowupRow[]>("/api/followups?projectId=" + encodeURIComponent(followupForm.projectId));
  } catch (e) {
    setError(e);
  }
}
async function loadProjectDetailFollowups(projectId: string) {
  ...
  try {
    projectDetailFollowups.value = await api<FollowupRow[]>("/api/followups?projectId=" + encodeURIComponent(projectId));
  } catch (e) {
    projectDetailFollowups.value = [];
    setError(e);
  }
...
async function loadContracts() {
  try {
    contracts.value = await api<ContractRow[]>("/api/contracts");
  } catch (e) {
    setError(e);
  }
}
...
async function loadPayments() {
  try {
    payments.value = await api<PaymentRow[]>("/api/payments");
  } catch (e) {
    setError(e);
  }
}
...
async function loadAudit() {
  try {
    auditLogs.value = await api<AuditRow[]>("/api/audit-logs");
  } catch (e) {
    setError(e);
  }
}
