...
| **签约** | 签订合同 | - | 创建合同记录、上传合同附件、录入合同金额 |
| **回款** | 按合同节点收款 | - | 录入实收记录、更新发票状态、维护回款凭证 |
...
1. 成员列表字段包含：姓名、登录手机号、部门、直属上级、所属角色、状态（启用/停用）
...
1. 用户仅支持“启用/停用”状态，V1 不做用户硬删除
2. 停用后不可登录
3. 可先停用，再批量转移其负责数据
4. 历史记录中的创建人、操作人、创建时间不得因成员状态变化而回写
5. 负责人转移支持跨部门，转移原因选填
...
1. 组织作为租户隔离边界
2. 成员由系统管理员在成员管理中维护状态（启用/停用）
3. 部门与上下级关系由系统内维护，用于“看下属”权限计算
