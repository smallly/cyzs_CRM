...
1. 系统管理员：固定 `ALL`
2. 招商人员：可配置 `ALL` / `SELF` / `SELF_AND_SUBORDINATES` / `DEPT` / `DEPT_AND_SUBTREE`

该范围统一作用于：查看、编辑、删除、转移负责人。
历史值 `SUBTREE` 兼容映射到 `DEPT_AND_SUBTREE`。
...
跟进记录与回款记录字段中“最后编辑时间”统一映射为 `updated_at`，新增时默认等于创建时间，后续编辑、软删除或业务日期调整时自动刷新。
