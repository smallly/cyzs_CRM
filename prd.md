...
5. 项目详情页进度条中的“签约日期/回款日期”展示规则为：分别取当前项目下合同、回款记录中的最早业务日期（首次签约/首次回款）。
6. 项目详情页进度条中的“签约日期/回款日期”不提供直接编辑入口；如需修改，必须进入对应合同或回款记录中调整业务单据日期。
...
| 合同标题 | 必填 | 合同标识名称 |
| 关联项目 | 必填 | 选择已创建且未删除的项目（一个项目可关联多份合同） |
| 合同金额 | 必填 | 合同总金额 |
| 签约日期 | 必填 | 合同签订时间 |
| 预估佣金（元） | 选填 | 预计佣金收益金额 |
| 起租日期 | 选填 | 租赁起始时间 |
...
6. **项目合同多份支持**：同一项目可创建多份合同，合同编号仍需保持组织内唯一
7. **进度条日期口径**：项目详情进度条中的“签约日期”取该项目全部合同中的最早 `signDate`，用于表达首次签约时间。
...
| 字段名称 | 数据库字段 | 后端字段 | 是否必填 | 说明 |
|---------|---------|---------|---------|------|
| 回款ID | `id` | `id` | 必填 | 系统生成的回款记录唯一标识 |
| 租户ID | `tenant_id` | `tenantId` | 必填 | 多租户隔离字段 |
| 关联合同 | `contract_id` | `contractId` | 必填 | 选择要记录回款的合同 |
| 负责人ID | `owner_id` | `ownerId` | 选填 | 当前负责人 |
| 创建人ID | `creator_id` | `creatorId` | 选填 | 创建该回款记录的用户 |
| 回款编号 | `code` | `code` | 必填 | 系统自动生成 |
| 回款日期 | `paid_date` | `paidDate` | 必填 | 实际收款时间 |
| 回款金额 | `amount` | `amount` | 必填 | 实际收款金额 |
| 付款方名称 | `payer_name` | `payerName` | 选填 | 付款单位或个人名称 |
| 开票状态 | `invoice_status` | `invoiceStatus` | 选填 | 已开票/未开票/不需开票 |
| 回款凭证 | `voucher` | `voucher` | 选填 | 上传回款凭证文件 |
| 备注 | `remark` | `remark` | 选填 | 其他说明 |
| 是否已删除 | `deleted` | `deleted` | 必填 | 软删除标记 |
| 创建时间 | `created_at` | `createdAt` | 必填 | 系统自动记录创建时间 |
| 最后编辑时间 | `updated_at` | `updatedAt` | 必填 | 系统自动记录最近一次编辑时间 |
| 删除时间 | `deleted_at` | `deletedAt` | 选填 | 软删除时间 |
...
4. **最后编辑时间**：每次编辑回款记录后，系统自动刷新“最后编辑时间”
5. **进度条日期口径**：项目详情进度条中的“回款日期”取该项目全部回款记录中的最早 `paidDate`，用于表达首次回款时间。
6. **修改入口约束**：项目详情进度条不直接编辑“回款日期”；如需调整，应在对应回款记录中修改业务日期。
...
| 功能项 | 功能描述 | 业务价值 |
|-------|---------|---------|
| **权限配置** | 设置角色数据范围配置、指定项目管理员 | 适配不同组织的管理风格 |
| **数据字典** | 自定义项目级别、项目来源的显示名称 | 适配组织内部术语习惯 |
| **成员管理** | 查看组织成员列表、维护成员状态（启用/停用）与所属角色 | 了解团队成员、处理账号启停与角色管理 |

#### 权限配置项

**项目数据范围配置**（角色级配置，V1 默认系统管理员固定全量、招商人员可配置）：

| 模式 | 名称 | 说明 | 适用场景 |
|------|------|------|----------|
| **全部数据** | 固定全量可见/可操作 | 系统管理员、项目管理员 | 全局管理角色 |
| **仅自己** | 成员仅可操作自己负责人范围内的数据 | 高隔离、强隐私场景（如个人业绩考核） | 个人作业 |
| **本人及下属** | 上级可操作全部下属（直接+间接）负责的数据 | 标准销售团队管理（团队协作场景） | 销售团队管理 |
| **本部门** | 当前部门内可操作 | 部门内部协作场景 | 组织层级管控 |
| **本部门及以下** | 当前部门及子树内可操作 | 区域/大区型组织管理 | 树形组织管理 |

**项目管理员指定**（具备权限配置菜单权限的角色可配置，V1 默认系统管理员）：

- 可指定多名项目管理员
- 项目管理员可查看本组织所有项目，不受角色数据范围配置限制
- 适用于销售总监、运营经理等全局管理角色
...
| 权限类型 | 控制范围 | 说明 |
|---------|---------|------|
| **业务数据权限** | 项目/联系人/合同等业务数据的可见性和操作权 | 基于"负责人"和"角色数据范围配置"判断谁能看/编辑/删除/转移 |
...
若配置为“本人及下属”，允许招商人员转移下属范围内数据的负责人。
...
| 我是招商人员且数据在我的可操作范围内 | ✅ 可操作 | 范围由角色数据范围配置决定 |
...
4. 部门与上下级关系由系统内维护，用于“本人及下属/本部门及以下”权限计算
...
| **一线招商人员** | 包含销售专员，专注于具体的客户拓展和项目跟进，受角色数据范围配置限制 |
| **管理层** | 包含项目管理员和系统管理员，负责全局把控、资源配置及系统运维 |
| **业务数据权限** | 控制用户对联系人、项目等业务数据的可见性和操作权，基于负责人和角色数据范围配置判断 |
| **系统功能权限** | 控制用户对系统菜单和配置功能的访问权，基于角色判断 |
| **项目数据范围配置** | 控制招商人员可操作的数据范围（全部/仅自己/本人及下属/本部门/本部门及以下） |
